...
export function Badge({ children, tone = 'neutral', solid = false, iconLeft }: BadgeProps) {
  ...
  const tones = {
    neutral: solid
      ? [theme.colors.textBody, theme.colors.textInverse]
      : [theme.colors.surfaceSunken, theme.colors.textBody],
    accent: solid
      ? [theme.colors.accent, theme.colors.textOnAccent]
      : [theme.colors.accentSoft, theme.colors.accentPress],
    success: solid
      ? [theme.colors.success, theme.colors.textInverse]
  ...
  return (
    <View style={[styles.badge, { backgroundColor }]}>
      {iconLeft}
      <Text style={{ color, fontFamily: theme.fonts.bodySemibold, fontSize: 12 }}>{children}</Text>
    </View>
  )
}
  ...
  return (
    <Text
      style={[
        styles.eyebrow,
        {
          color: inverse ? theme.colors.accent : theme.colors.accentPress,
