  ...
  return (
    <View
      style={[
        styles.card,
        {
          backgroundColor: invert ? theme.colors.bandBorder : theme.colors.surfaceCard,
          borderColor: invert ? theme.colors.bandBorder : theme.colors.border,
        },
        !invert && theme.shadows.sm,
      ]}
    >
      <View style={styles.stars}>
        {Array.from({ length: 5 }, (_, index) => (
          <Star
            key={index}
            fill={index < rating ? theme.colors.accent : 'transparent'}
...
const styles = StyleSheet.create({
  card: { borderRadius: 16, borderWidth: 1, gap: 16, minHeight: 260, padding: 32 },
  stars: { flexDirection: 'row', gap: 3 },
  quote: { fontSize: 20, lineHeight: 29 },
  person: { gap: 2, marginTop: 'auto' },
})
